...
class CModel:
    ...
    def simulate(self, t_0,t_1, dt, IC, S_e, S_i, I_inj=[0.0], G_inj = [0.0], E = -80, Iinj_site = 0, Ginj_site = 0, break_flag=False):
        ...
        I_inj : scalar/ndarray (nA)
            injected current at soma (default 0)
        G_inj : scalar/ndarray (uS)
            injected conductance at soma (default 0)
        E: scalar
            reversal of the injected conductance 
        inj_site: int
        ...
        a_ion = [[[self.P['g_na']], [self.P['g_k']], [self.P['g_m']] ,[self.P['g_ca']], [self.P['g_kca']],[self.P['g_nad']]],
        ...
        dist = self.P['dist']
        dist_temp = []
        for i, d1 in enumerate(self.P['dist']):
            dist_temp.append(d1)
            if len(np.asarray([d1]).shape)>1:
                idx_nonuniform_dist.append(i)
                dist[i] = dist[i][0]
            else:
                dist[i] = dist[i]
        ...
        a_temp = a_ion
        idx_nonuniform = []
        for i,a1 in enumerate(a_ion):
            for j, a2 in enumerate(a_ion[i]):
                if len(np.asarray(a2).shape)>1:
                    idx_nonuniform.append([i,j])
                    a_temp[i][j] = a_ion[i][j][0][0]
                else:
                    a_temp[i][j] = a_ion[i][j][0]

        g_ion = np.asarray(a_temp)
        ...
        if len(np.asarray(I_inj).shape)>1:
            ...
            for i in range(I_inj.shape[0]):
                I_inj[i,:] *= 1/(self.area[Iinj_site[i]])*1e-5
        else:
            I_inj *= 1/(self.area[Iinj_site])*1e-5  # uA/cm^2

        if len(np.asarray(G_inj).shape)>1:
            G_inj = np.asarray(G_inj)
            for i in range(G_inj.shape[0]):
                G_inj[i,:] *= 1/(self.area[Ginj_site[i]])*1e-5
        else:
            G_inj *= 1/(self.area[Ginj_site])*1e-5  # mS/cm^2
        ...
        for k in range(1, len(t)):
            if len(idx_nonuniform_dist)>0:
                for idx in idx_nonuniform_dist:
                    dist[idx] = dist_temp[idx][k]
                    channels[5] = nad(v_0, dist = dist, N = N, temp = temp)
            ...
            if len(np.asarray(I_inj).shape)>1:
                ...
                f[Iinj_site] += I_inj
            if len(np.asarray(G_inj).shape)>1:
                f[Ginj_site] += G_inj[:,k]*(E-v[Ginj_site, k - 1])
            else:
                f[Ginj_site] += G_inj*(E-v[Ginj_site, k - 1])
            f *= dt/cm
            ...
            if len(idx_nonuniform)>0:
                for idx in idx_nonuniform:
                    g_ion[idx[1]][idx[0]] = a_ion[idx[0]][idx[1]][0][k]
